...
def new_game():
    global WIDTH, HEIGHT, time, score, lives, started, rock_group, missile_group
    WIDTH = 800
    HEIGHT = 600
    time = 0
    score = 0
...
class Sprite:
    ...
    def collide(self,  other_object):
        if self.radius + other_object.get_radius() > dist(self.pos, other_object.get_position()):
            return True
        else:
            return False
...
def group_group_collide(group_1, group_2):
    # group_1 is missile; group_2 is rock
    num = 0
    for item in set(group_1):
        res = group_collide(group_2, item)
        num += res
        if res > 0:
